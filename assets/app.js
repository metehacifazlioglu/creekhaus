...
const ICONS = {
  ...
  cloud:S('<path d="M7 18h10a4 4 0 0 0 .5-7.97A6 6 0 0 0 6 9.5 3.5 3.5 0 0 0 7 18Z"/>'),
...
/* ===== v4: task execution + purchasing ===== */
document.addEventListener('DOMContentLoaded',()=>{
  function updateTaskProgress(task){
    const total=task.querySelectorAll('.check').length;
    const done=task.querySelectorAll('.check.done').length;
    const bar=task.querySelector('.task-progress .bar i');
    const lbl=task.querySelector('.task-progress .lbl');
  ...
  document.querySelectorAll('[data-task]').forEach(task=>{
    ...
    task.querySelectorAll('.check').forEach(c=>c.addEventListener('click',()=>setTimeout(()=>updateTaskProgress(task),0)));
  });
  document.querySelectorAll('[data-task-submit]').forEach(btn=>{
    btn.addEventListener('click',()=>{ btn.innerHTML='<i data-i="check"></i> Task Completed'; paintIcons(btn); btn.style.opacity='.9'; toast('Task submitted'); });
  });
  // purchasing
  document.querySelectorAll('.supplier-opt').forEach(o=>o.addEventListener('click',()=>{
    o.parentElement.querySelectorAll('.supplier-opt').forEach(x=>x.classList.remove('sel')); o.classList.add('sel');
  }));
  const addItemBtn=document.querySelector('[data-add-item]');
  if(addItemBtn) addItemBtn.addEventListener('click',()=>{
    const wrap=document.querySelector('[data-item-list]'); const input=document.querySelector('[data-item-input]'); const qty=document.querySelector('[data-qty-input]');
    if(!input||!input.value.trim()) return;
    const row=document.createElement('div'); row.className='pitem';
    row.innerHTML='<span class="nm">'+input.value.trim()+'</span>'+((qty&&qty.value.trim())?'<span class="qty">'+qty.value.trim()+'</span>':'')+'<button class="x"><i data-i="x"></i></button>';
    wrap.appendChild(row); paintIcons(row); input.value=''; if(qty) qty.value='';
  });
  document.addEventListener('click',e=>{ const x=e.target.closest('.pitem .x'); if(x) x.closest('.pitem').remove(); });
  document.querySelectorAll('[data-purchase-submit]').forEach(btn=>btn.addEventListener('click',()=>toast('Purchase recorded')));
});
